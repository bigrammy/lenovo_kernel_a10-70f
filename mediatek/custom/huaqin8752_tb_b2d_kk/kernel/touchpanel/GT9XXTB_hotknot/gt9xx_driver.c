...
static int tpd_def_calmat_local_normal_wxga[8]  = TPD_CALIBRATION_MATRIX_ROTATION_NORMAL_WXGA;
static int tpd_def_calmat_local_factory_wxga[8] = TPD_CALIBRATION_MATRIX_ROTATION_FACTORY_WXGA;
static int tpd_def_calmat_local_normal_qxga[8]  = TPD_CALIBRATION_MATRIX_ROTATION_NORMAL_QXGA;
static int tpd_def_calmat_local_factory_qxga[8] = TPD_CALIBRATION_MATRIX_ROTATION_FACTORY_QXGA;
#endif

u8 cfg_info_group1_DFT[] = CTP_CFG_GROUP1;
u8 cfg_info_group1_WXGA[] = CTP_CFG_GROUP1_WXGA;
u8 cfg_info_group1_QXGA[] = CTP_CFG_GROUP1_QXGA;
...
static s32 gtp_init_panel(struct i2c_client *client)
{
    ...
    u8 *cfg_info_group1 = cfg_info_group1_DFT;
    ...
    u8 cfg_info_len[] = { CFG_GROUP_LEN(cfg_info_group1_DFT), 
    ...
    if(0 == strncmp(LCM_WIDTH, "1280", 4))
    {
        send_cfg_buf[0] = cfg_info_group1 = cfg_info_group1_WXGA;
        cfg_info_len[0] = CFG_GROUP_LEN(cfg_info_group1_WXGA);
    }
    else if(0 == strncmp(LCM_WIDTH, "1536", 4))
    {
        send_cfg_buf[0] = cfg_info_group1 = cfg_info_group1_QXGA;
        cfg_info_len[0] = CFG_GROUP_LEN(cfg_info_group1_QXGA);
    }
...
static int tpd_local_init(void)
{
	...
	GTP_DEBUG("LCM_WIDTH=%s\n", LCM_WIDTH);
    if(0 == strncmp(LCM_WIDTH, "1280", 4))
    {
    	if (FACTORY_BOOT == get_boot_mode())
	    {
	        GTP_DEBUG("Factory (LCM_WIDTH_1280) mode is detected! \n");
	        memcpy(tpd_calmat, tpd_def_calmat_local_factory_wxga, 8 * 4);
	        memcpy(tpd_def_calmat, tpd_def_calmat_local_factory_wxga, 8 * 4);
	    }
	    else
	    {
	        GTP_DEBUG("Normal (LCM_WIDTH_1280) mode is detected! \n");
	        memcpy(tpd_calmat, tpd_def_calmat_local_normal_wxga, 8 * 4);
	        memcpy(tpd_def_calmat, tpd_def_calmat_local_normal_wxga, 8 * 4);
	    }
    }
    else if(0 == strncmp(LCM_WIDTH, "1536", 4))
    {
    	if (FACTORY_BOOT == get_boot_mode())
	    {
	        GTP_DEBUG("Factory (LCM_WIDTH_2048) mode is detected! \n");
	        memcpy(tpd_calmat, tpd_def_calmat_local_factory_qxga, 8 * 4);
	        memcpy(tpd_def_calmat, tpd_def_calmat_local_factory_qxga, 8 * 4);
	    }
	    else
	    {
	        GTP_DEBUG("Normal (LCM_WIDTH_2048) mode is detected! \n");
	        memcpy(tpd_calmat, tpd_def_calmat_local_normal_qxga, 8 * 4);
	        memcpy(tpd_def_calmat, tpd_def_calmat_local_normal_qxga, 8 * 4);
	    }
    }
    else
    {

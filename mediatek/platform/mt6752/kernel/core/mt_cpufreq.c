...
static void _mt_cpufreq_early_suspend(struct early_suspend *h)
{
    ...
    if (gEMI_DFS_enable && gMd32_alive && gPMIC_6325_version >= 0x2550)
    {
        //switch to 1 PLL
        do_DRAM_DFS(0);
...
static int cpufreq_emi_dfs_proc_show(struct seq_file *m, void *v)
{
    ...
    seq_printf(m, "EMI DFS: %s, %s, EMI_DFS_e=%d, MD32_a=%d, PMIC_v=0x%x\n", 
        (gEMI_DFS_enable && gMd32_alive && gPMIC_6325_version >= 0x2550)?"enable":"disable", (mt_get_clk_mem_sel()==1)?"3PLL":"1PLL", 
